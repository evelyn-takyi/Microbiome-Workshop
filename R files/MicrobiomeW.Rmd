...
p.ss.boxplot <- ggplot(ss.df, aes_string(x, y = "No of reads", color = "Treatment")) + 
...
ps2.T6 <- prune_taxa(taxa_sums(ps2.T6) > 0, ps2.T1)
...
```{r}
Raf <-ggrare(ps2, step = 50, color="ProjectName",  se = TRUE)
```
### Alpha diversity
...
Order_plot <- ggplot(Order1d_count)+
  ...
  facet_grid(.~Type+Treatment+Name, drop=TRUE,scale="free", space="free",switch="x")+
...
Read_plot <-ggplot(Meta_T,aes(SampleID,sum, fill=Treatment))+
  ...
  facet_grid(.~Type+Treatment+Name, drop=TRUE, scale="free_x")+
  # label the axes
  labs(y="Read Abundance", x=NULL)+
  # change the fill color to Control= light blue, Treatment=dark red.
  scale_fill_manual(values=c("#00545b","#ff856d","#640025","#3ddda5","#cdffaa","#150e00","#bae278"))+
