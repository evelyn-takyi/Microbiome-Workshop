...
levels(sample_data(phy)$ProjectName)
sample_data(phy)$ProjectName <- factor(sample_data(phy)$ProjectName, levels = c("Trial6", "T7"))
levels(sample_data(phy)$ProjectName)
```
...
```{r}
# Trial6
ps2.T6 <- subset_samples(ps2, ProjectName == "Trial6")
...
# Trial7
ps2.T7 <- subset_samples(ps2, ProjectName == "T7")
...
```
### Alpha diversity
###### Alpha diversity is a standard tool researchers can use to calculate the number of bacterial taxa present in a study or study group and the relationships between relative abundance and how evenly taxa are distributed. 
```{r}
SD <- sample_data(ps2) %>%
    data.frame() %>%
    select("Treatment","ProjectName","Type", "Name") %>%
...
Div_SIM <- ggplot(adiv, aes(as.factor(Name), Simpson)) +
  facet_grid(.~ProjectName, scales="free",space="free_x",switch="y")+
